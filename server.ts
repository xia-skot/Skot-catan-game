...
async function startServer() {
  ...
  app.get('/api/proxy-image', async (req, res) => {
    try {
      ...
      const encodedUrl = url.origin + url.pathname.split('/').map(p => encodeURIComponent(p)).join('/') + url.search;

      const response = await fetch(encodedUrl);
      
      if (!response.ok) {
        console.error(`Failed to fetch image: ${response.status} ${response.statusText}`, encodedUrl);
