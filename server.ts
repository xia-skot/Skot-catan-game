...
async function startServer() {
  ...
  const initTransporter = async () => {
    ...
    try {
      ...
      console.log(`[Server] [Network Fix] Resolved ${smtpHost} to IPv4: ${resolvedAddress}`);
      ...
      const t = nodemailer.createTransport({
        ...
        // 当使用 IP 连接时，必须通过 tls.servername 告知证书应该校验哪个域名
        tls: {
          servername: smtpHost,
      ...
      await t.verify();
      console.log('[Server] SMTP Server is authenticated and READY via IPv4');
      return t;
    } catch (err: any) {
      console.error('[Server] SMTP Initialization FAILED:', err.message);
      return null;
    }
  };
