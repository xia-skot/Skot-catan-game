...
// 强制 Node.js 在 Render 环境下优先使用 IPv4，解决 ENETUNREACH IPv6 错误
...
async function startServer() {
  ...
  const transporter = nodemailer.createTransport({
    ...
    // 强制使用 IPv4，防止 Render 环境下 IPv6 连接超时/拒绝
    // @ts-ignore
    family: 4,
    // 增加连接池设置，提高稳定性
    pool: true,
  ...
  if (smtpUser && smtpPass) {
    transporter.verify((error, success) => {
      if (error) {
        ...
        console.error('[Server] Hint: Check if SMTP_HOST matches your email provider (e.g., smtp.163.com)');
      } else {
        console.log('[Server] SMTP Server is authenticated and READY');
      }
    });
  }
  ...
  app.post('/api/send-code', async (req, res) => {
    ...
    try {
      ...
      if (!smtpUser || !smtpPass) {
        console.log('[Server] No SMTP configured, using fallback 123456');
        return res.json({ message: '测试环境：没有配置 SMTP，邮件系统已默认使用验证码 123456' });
      }
