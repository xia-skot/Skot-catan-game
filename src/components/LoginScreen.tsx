  ...
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/guest', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: gUsername })
      });
      const data = await res.json();
      ...
      >
        <div className="text-center mb-8">
          <div className="w-16 h-16 mx-auto mb-4 bg-slate-50 rounded-2xl flex items-center justify-center shadow-inner border border-slate-100">
            <img src="/api/proxy-image?url=http%3A%2F%2Ftdyuzzmmy.hn-bkt.clouddn.com%2Fimg%2FC3eURTs%20-%20Imgur.png" alt="Logo" className="w-10 h-10 object-contain drop-shadow-lg" />
          </div>
