...
export default function App() {
  ...
  if (!isJoinedLobby) {
    return (
      <div className="flex flex-col h-screen w-full bg-slate-50 font-sans items-center justify-center p-2 sm:p-4 relative selection:bg-indigo-600 selection:text-white overflow-hidden">
        {isPortrait && isMobile && <OrientationOverlay />}
  ...
  if (!gameStarted) {
    return (
      <div className="flex flex-col sm:flex-row h-screen w-full bg-[#f8fafc] font-sans overflow-hidden relative selection:bg-indigo-600 selection:text-white">
        {isPortrait && isMobile && <OrientationOverlay />}
          ...
          <div className="w-full max-w-lg space-y-2 sm:space-y-3 px-1 lg:px-2 flex-1 flex flex-col justify-center min-h-0">
            ...
            <div className="flex flex-col gap-1.5 sm:gap-2 pt-1">
              ...
              {isHostInLobby && (
                ...
                >
                  <span className="relative z-10">{roomState?.players.every(p => p.isReady) ? '开启宏伟旅程' : '等待全体领主准备'}</span>
                </button>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 sm:gap-4 w-full max-w-sm mx-auto h-full">
            ...
            <div className="space-y-2 sm:space-y-3">
              <div className="flex items-center justify-between px-1">
                ...
                <div className="flex items-center gap-1 px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-full border border-emerald-100 shadow-sm">
                  ...
                  <span className="text-[7px] sm:text-[8px] font-black uppercase tracking-widest">{roomState?.players.length || 0} / {playerCount}</span>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-1.5 flex-1 overflow-y-auto pr-1 no-scrollbar">
              ...
              {Array.from({ length: Math.max(0, playerCount - (roomState?.players.length || 0)) }).map((_, i) => {
                ...
                return (
                  <div key={`empty-${i}`} className={`flex items-center justify-between p-2.5 sm:p-3 rounded-xl border transition-all duration-300 ${botConfig[globalIndex] ? 'bg-white border-indigo-100 shadow-lg shadow-indigo-50/50' : 'border-dashed border-slate-200 opacity-40 hover:opacity-100 hover:border-indigo-200 group'}`}>
                    <div className="flex items-center gap-2.5">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${botConfig[globalIndex] ? 'bg-indigo-50 border border-indigo-100' : 'border border-dashed border-slate-200 group-hover:bg-slate-50'}`}>
                        {botConfig[globalIndex] ? <Bot size={14} className="text-indigo-600" /> : <Users size={12} className="text-slate-300" />}
                      </div>
                      <div className="flex flex-col">
                        <span className={`text-[11px] font-black transition-colors ${botConfig[globalIndex] ? 'text-slate-800' : 'text-slate-400'}`}>{botConfig[globalIndex] ? '领主 AI' : '未占领席位'}</span>
                        {botConfig[globalIndex] && <span className="text-[6px] font-bold text-indigo-400 uppercase tracking-widest">高级AI</span>}
                      </div>
                    </div>
                    {isHostInLobby && (
                      <button 
  ...
  if (!gameState) return (
    <div className="flex flex-col items-center justify-center h-[100dvh] bg-[#0a0a0a] text-white relative overflow-hidden">
      {isPortrait && isMobile && <OrientationOverlay />}
  ...
  const leftWidth = isMobile ? windowSize.width * 0.16 : 280;
  const rightWidth = isMobile ? windowSize.width * 0.18 : 280;
  ...
  const nextAction = (() => {
    ...
    if (activePlayerId === myPlayerIndex) {
      ...
      if (gameState.phase === 'road_building') return "建设道路";
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#f5f2ed] text-[#1a1a1a] overflow-hidden font-sans selection:bg-black selection:text-white relative">
      ...
      <header className="w-full flex items-center bg-white border-b border-black/5 px-2 z-50 overflow-hidden" style={{ height: headerHeight }}>
        ...
        {/* Center: Player Cards */}
        <div className="flex-1 flex justify-center overflow-x-auto no-scrollbar py-1 lg:ml-32 lg:mr-32 no-scrollbar">
              ...
              <div 
                ...
                <div className="flex flex-col flex-1 pl-0.5">
                    <div className="flex items-center gap-1">
                      ...
                      {((gameState.phase === 'initial_dice_roll' || gameState.phase === 'order_determination') && gameState.initialDiceRolls[i]) ? (
                        <div className="flex items-center gap-0.5 px-1 rounded-sm bg-orange-500/10 border border-orange-500/20">
                          <span className="text-[8px] font-black text-orange-600">{String((gameState.initialDiceRolls[i][0] || 0) + (gameState.initialDiceRolls[i][1] || 0))}</span>
                        </div>
                      ) : (isCurrent && gameState.hasRolled && gameState.dice && gameState.dice[0] > 0) ? (
                        <div className="flex items-center gap-0.5 px-1 rounded-sm bg-orange-500/10 border border-orange-500/20">
                          <span className="text-[8px] font-black text-orange-600">{String((gameState.dice[0] || 0) + (gameState.dice[1] || 0))}</span>
                        </div>
                      ) : null}
                    </div>
                    <div className="flex items-center mt-0.5 leading-none">
                      ...
                      <span className={`flex items-center gap-0.5 ${isMobile ? 'text-[8px]' : 'text-[10px]'} font-mono opacity-80 whitespace-nowrap ml-1`} title="道路">
            ...
            <div className={`grid grid-cols-3 grid-rows-2 ${isMobile ? 'gap-0.5' : 'gap-1'}`}>
              {Object.entries(gameState.bankResources).map(([res, count]) => (
                <div key={res} className={`flex items-center justify-between ${isMobile ? 'p-0.5 px-1' : 'p-1.5'} rounded-md bg-stone-100/50 border border-black/10`}>
                  <div className={`${isMobile ? 'w-1 h-1' : 'w-2 h-2'} rounded-full`} style={{ backgroundColor: RESOURCE_COLORS[res as ResourceType] }} />
                  ...
                  {me.devCards.length > 0 && (
                    <div className="space-y-2">
                      ...
                      {Object.values(DevCardType).map(type => {
                        ...
                        return (
                          <div key={`playable-${type}`} className="flex flex-col p-1.5 rounded-lg bg-white border border-black/5 shadow-sm lg:max-h-16">
                            <div className="flex items-center justify-between mb-1">
                              <div className="flex items-center gap-1.5">
                                <div className="w-5 h-5 rounded-md bg-zinc-100 flex items-center justify-center text-[8px] relative">
                                  {type === DevCardType.Knight ? '⚔️' : 
                                   type === DevCardType.VictoryPoint ? '🏆' :
                                   type === DevCardType.RoadBuilding ? '🛣️' :
                                   type === DevCardType.YearOfPlenty ? '🎁' : '💎'}
                                  {count > 1 && (
                                    <div className="absolute -top-1.5 -right-1.5 w-3 h-3 sm:w-4 sm:h-4 bg-black text-white rounded-full flex items-center justify-center text-[7px] sm:text-[8px] font-bold">
                                      {count}
                                    </div>
                                  )}
                                </div>
                              ...
                              {type !== DevCardType.VictoryPoint && (
                                <div className="flex items-center gap-1.5 sm:gap-2">
                                  {gameState.playingDevCard === type ? (
                                    <button 
                                      onClick={cancelDevCard}
                                      className="text-[7px] sm:text-[8px] font-black uppercase tracking-widest bg-red-500 text-white px-2 py-1 sm:px-3 sm:py-1.5 rounded-full hover:bg-red-600 transition-all shadow-md active:scale-95"
                                    >
                                      取消
                                    </button>
                                  ) : (
                                    <button 
                                      ...
                                      className="text-[7px] sm:text-[8px] font-black uppercase tracking-widest bg-black text-white px-2 py-1 sm:px-3 sm:py-1.5 rounded-full hover:bg-zinc-800 transition-all shadow-md active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed"
                                    >
                                      使用
                                    </button>
                                  )}
      ...
      <AnimatePresence>
        {showDebug && (
          <motion.div
            ...
            className="fixed top-20 right-4 bg-white/90 backdrop-blur-xl p-6 rounded-3xl shadow-2xl border border-black/5 w-80 z-50 max-h-[80vh] overflow-y-auto no-scrollbar"
          >
        ...
        {showPlayerTradeModal && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="bg-white rounded-3xl p-5 max-w-lg w-full max-h-[85vh] overflow-hidden flex flex-col"
            >
                  ...
                  <div className="grid gap-1.5 grid-cols-2 md:grid-cols-5">
                    {Object.values(ResourceType).map(r => (
                      <div key={`offer-${r}`} className="p-2 border rounded-lg flex flex-col items-center gap-1">
                        <div className="w-4 h-4 rounded-full shadow-sm" style={{ backgroundColor: RESOURCE_COLORS[r] }} />
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {Object.values(ResourceType).map(r => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div className="w-4 h-4 rounded-full shadow-inner" style={{ backgroundColor: RESOURCE_COLORS[r] }} />
        ...
        <div className="flex gap-1.5">
          {Object.entries(cost).map(([res, amt]) => (
            <div key={res} className="flex flex-col items-center gap-0.5">
              <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: RESOURCE_COLORS[res as ResourceType] }} />
