...
export default function App() {
  ...
  // Keep-alive ping for Render/free-tier hosting (every 5 mins)
  useEffect(() => {
    const keepAliveInterval = setInterval(() => {
      fetch('/api/health').catch(e => console.error('Keep-alive failed', e));
    }, 5 * 60 * 1000);
  ...
  if (isCheckingSession) {
    return (
      <div style={lockedLandscapeStyle}>
        {isPortrait && (
          <div className="fixed inset-0 z-[999] bg-slate-950 flex flex-col items-center justify-center text-white px-8 text-center backdrop-blur-md">
            <motion.div
            ...
            <h2 className="text-3xl font-serif font-black italic mb-4 tracking-tight leading-none">请旋转屏幕</h2>
            <p className="text-sm opacity-60 max-w-[280px] leading-relaxed font-medium">
              为了获得最佳的重连体验，请开启自动旋转并使用横屏进行游戏。
            </p>
          </div>
        )}
          ...
          <div className="flex flex-col gap-2 sm:gap-3 text-left w-full">
            ...
            <div className="group">
              ...
              {localStorage.getItem('catan_active_room') ? (
                <div className="w-full bg-indigo-50 border-2 border-indigo-100 px-3 py-2 sm:py-3 rounded-xl font-black font-mono tracking-[0.2em] text-center text-indigo-900 text-xs sm:text-sm shadow-sm flex flex-col items-center gap-1.5 ring-4 ring-indigo-500/5">
                  <div className="flex items-center gap-2">
                    <Lock size={12} className="text-indigo-500" />
                    <span className="text-lg">{localStorage.getItem('catan_active_room')}</span>
                  </div>
                  <div className="text-[7px] text-indigo-500/60 uppercase font-black tracking-[0.3em]">海域已锁定 · 正在征服中</div>
                </div>
              ) : (
                <input 
                  type="text" 
            ...
            >
              <span className="relative z-10">{localStorage.getItem('catan_active_room') ? '重新连接' : '进入海域'}</span>
          ...
          <div className="w-full max-w-lg space-y-2 sm:space-y-3 px-1 lg:px-2 flex flex-col justify-end min-h-0">
            ...
            <div className={!isHostInLobby ? 'opacity-70 pointer-events-none' : ''}>
              ...
              <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
                {[
                  ...
                  <button
                    key={map.id}
                    onClick={() => { setMapType(map.id as MapType); syncSettings({ mapType: map.id, customBoard: undefined, customMapName: undefined }); }}
                  ...
                  >
                    <div className={`w-16 h-12 sm:w-24 sm:h-20 lg:w-32 lg:h-24 relative overflow-hidden flex items-center justify-center transition-transform duration-500 ${mapType === map.id ? 'scale-110' : ''}`}>
                      <div className="absolute inset-x-0 inset-y-[-20%] pointer-events-none">
                        <MapPreview board={map.board} isTopologyOnly={true} />
                      </div>
                    </div>
            ...
            {isHostInLobby && (
              <button
                ...
                className="w-full flex items-center justify-between mt-2 p-2 sm:p-3 bg-indigo-50 hover:bg-indigo-100 border border-indigo-100 rounded-lg lg:rounded-xl transition-all"
              >
                <div className="flex items-center gap-2">
                  <span className="text-sm">🧭</span>
                  <div className="flex flex-col text-left">
                    <span className="text-[10px] font-black uppercase tracking-widest text-indigo-900">地图收藏册</span>
                    <span className="text-[8px] font-bold tracking-widest text-indigo-500/70">
                      {roomState?.settings?.customMapName ? `当前选择: ${roomState.settings.customMapName}` : '使用已收藏的地图，或生成全新地图'}
                    </span>
                  </div>
                </div>
            ...
            <div className="flex flex-col gap-1.5 sm:gap-2 pt-1">
              {!roomState?.players.find(p => p.id === socketService.playerId)?.isReady ? (
                <button 
                  ...
                  className={`relative z-50 w-full bg-emerald-600 text-white py-2.5 sm:py-3.5 rounded-lg lg:rounded-xl font-black uppercase tracking-[0.2em] shadow-lg shadow-emerald-200 hover:bg-emerald-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-[10px] sm:text-[11px] disabled:opacity-30 disabled:cursor-not-allowed disabled:grayscale ${isSpectator ? 'opacity-50 grayscale' : ''}`}
                >
                ...
                <button 
                  onClick={handleToggleReady}
                  disabled={isSpectator}
                  className={`w-full bg-white text-slate-500 py-2.5 sm:py-3.5 rounded-lg lg:rounded-xl font-black uppercase tracking-[0.2em] border-2 border-slate-100 hover:bg-slate-50 transition-all flex items-center justify-center gap-2 text-[10px] ${isSpectator ? 'opacity-50 grayscale cursor-not-allowed' : ''}`}
                >
                    ...
                    <div className="relative inline-block mt-1">
                      <select 
                        ...
                        onChange={e => {
                          ...
                          syncSettings({ playerCount: newCount, customBoard: undefined, customMapName: undefined });
                        }}
      ...
      <div style={lockedLandscapeStyle}>
        {isPortrait && (
          <div className="fixed inset-0 z-[999] bg-slate-950 flex flex-col items-center justify-center text-white px-8 text-center backdrop-blur-md">
            ...
            <h2 className="text-3xl font-serif font-black italic mb-4 tracking-tight leading-none">请旋转屏幕</h2>
            <p className="text-sm opacity-60 max-w-[280px] leading-relaxed font-medium">
              为了获得最佳的游戏生成体验，请开启自动旋转并使用横屏进行游戏。
            </p>
          </div>
        )}
        <div className="flex flex-col items-center justify-center h-full w-full bg-[#0a0a0a] text-white relative overflow-hidden">
    ...
    <div style={lockedLandscapeStyle}>
      {isPortrait && (
        <div className="fixed inset-0 z-[999] bg-slate-950 flex flex-col items-center justify-center text-white px-8 text-center backdrop-blur-md">
          ...
          <h2 className="text-3xl font-serif font-black italic mb-4 tracking-tight leading-none">请旋转屏幕</h2>
          <p className="text-sm opacity-60 max-w-[280px] leading-relaxed font-medium">
            海域广阔，为了获得最佳的征服体验，请开启自动旋转并使用横屏进行游戏。
          </p>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <main className="flex-1 flex relative overflow-hidden bg-[#e4e3e0]">
          ...
          {gameStarted && (
            <div className={`absolute top-1 left-1 z-[1000] flex flex-col gap-0.5 pointer-events-auto ${isMobile ? 'scale-90 origin-top-left' : ''}`}>
              <button 
                onClick={handleReturnToLobby}
                className={`flex items-center gap-2 ${isMobile ? 'px-3 py-2 text-[11px] rounded-lg' : 'px-6 py-3 text-[14px] rounded-xl'} bg-white border-2 border-black/5 ${isSpectator ? 'text-red-500 bg-white shadow-red-500/10' : 'text-stone-600 bg-white/90 backdrop-blur-xl'} font-black uppercase tracking-widest shadow-2xl hover:bg-stone-100 transition-all transform active:scale-95 group pointer-events-auto`}
                style={{ cursor: 'pointer' }}
              >
                <LogOut size={isMobile ? 14 : 18} className="group-hover:-translate-x-0.5 transition-transform scale-x-[-1]" />
                <span>{isSpectator ? '退出观战' : '中途离开'}</span>
              </button>

              {isHost && (
                <button 
                  onClick={() => setShowDissolveRoomConfirm(true)}
                  className={`flex items-center gap-2 ${isMobile ? 'px-3 py-2 text-[11px] rounded-lg' : 'px-6 py-3 text-[14px] rounded-xl'} bg-white/90 backdrop-blur-xl border border-black/5 text-red-500 font-bold uppercase tracking-widest shadow-xl hover:bg-red-500 hover:text-white transition-all transform active:scale-95 group`}
                >
      ...
      <Stage width={dimensions.width} height={dimensions.height}>
        <Layer x={dimensions.width / 2} y={dimensions.height / 2} scale={{ x: scale, y: scale }}>
          {hexCoords.map(hex => {
            let fill = '#fff';
            if (isTopologyOnly) {
              if (hex.type === HexType.Gold) fill = '#ffd700'; // 金矿 (Gold)
              else if (hex.type === HexType.Desert || hex.category === 'Desert' || hex._category === 'Desert') fill = '#dca467'; // 沙漠 (Desert)
              else if (hex.category === 'Mainland' || hex._category === 'Mainland' || hex.isMainland) fill = '#558b2f'; // 大陆 (Mainland) - Deeper Green
              else if (hex.category === 'Island' || hex._category === 'Island' || hex.isIsland) fill = '#7cb342'; // 岛屿 (Island) - Yellowish Green
