...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Auto-join if activeRoom exists
    if (activeRoom) {
      setIsCheckingSession(true);
      // Wait for socket to be ready then join
      setTimeout(() => {
    ...
    <div style={lockedLandscapeStyle}>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <main className="flex-1 flex relative overflow-hidden bg-[#e4e3e0]">
          ...
          {gameStarted && gameState && gameState.players[activePlayerId] && (isSpectator || myPlayerIndex === activePlayerId) && (
            <div 
              className="absolute inset-0 pointer-events-none z-[45] animate-[breatheglow_4s_ease-in-out_infinite] transition-colors duration-1000"
              style={{
                boxShadow: `inset 0 0 30px -5px ${gameState.players[activePlayerId].color}30, inset 0 0 10px -2px ${gameState.players[activePlayerId].color}40`,
                border: `3px solid ${gameState.players[activePlayerId].color}60`
              }}
            />
          )}
...
function HexCell({ hex, isSelected, isRobber, isPirate, onClick }: { hex: any, isSelected: boolean, isRobber: boolean, isPirate: boolean, onClick: () => void }) {
  ...
  useEffect(() => {
    ...
    switch (hex.type) {
      ...
      case HexType.Gold: src = '/api/proxy-image?url=' + encodeURIComponent('http://tdyuzzmmy.hn-bkt.clouddn.com/img/金矿2.jpg'); break;
