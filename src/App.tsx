          ...
          {gameStarted && (
            <div className="absolute top-1 left-1 z-40 flex flex-col gap-0.5">
              ...
              <button 
                ...
                className={`flex items-center gap-2 ${isMobile ? 'px-2 py-1.5 text-[10px] rounded-md' : 'px-6 py-3 text-[14px] rounded-xl'} bg-indigo-50/90 backdrop-blur-xl border border-indigo-100 text-indigo-600 font-bold uppercase tracking-widest shadow-xl hover:bg-indigo-100 transition-all transform active:scale-95 group`}
                title="重新向服务器请求最新状态，解决可能的不同步问题"
              >
                <RefreshCw size={isMobile ? 12 : 18} className="group-hover:rotate-180 transition-transform duration-500" />
                <span>同步状态</span>
              </button>
            </div>
          )}
