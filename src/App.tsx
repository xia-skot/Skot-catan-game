...
const RobberToken = ({ x, y, isPhaseRobber }: { x: number, y: number, isPhaseRobber: boolean }) => {
  ...
  useEffect(() => {
    ...
    image.src = '/api/proxy-image?url=' + encodeURIComponent('http://tdyuzzmmy.hn-bkt.clouddn.com/img/强盗2.png');
...
const PirateToken = ({ x, y, isPhaseRobber }: { x: number, y: number, isPhaseRobber: boolean }) => {
  ...
  useEffect(() => {
    const image = new window.Image();
    image.src = '/api/proxy-image?url=' + encodeURIComponent('http://tdyuzzmmy.hn-bkt.clouddn.com/img/海盗船3.png');
    image.referrerPolicy = 'no-referrer';
    image.crossOrigin = 'Anonymous';
    image.onload = () => setImg(image);
  }, []);
            ...
            {wavePaths.line && (
                ...
                <div 
                    ...
                    <div style={{ transform: 'translate(-50%, -95%)', width: boatSize, height: boatSize }} className="drop-shadow-[0_10px_20px_rgba(0,0,0,0.5)]">
                        <img src="/api/proxy-image?url=http%3A%2F%2Ftdyuzzmmy.hn-bkt.clouddn.com%2Fimg%2F%E5%B8%86%E8%88%B9.png" alt="Sailing Boat" className="w-full h-full object-contain" />
                    </div>
                </div>
...
export default function App() {
  ...
  useEffect(() => {
    return socketService.onConnectionChange((connected) => {
      ...
      if (connected && isJoinedLobby && !isAuthLoading) {
        const roomId = roomState?.roomId || inputRoomId;
        if (roomId) {
  ...
  }, [isJoinedLobby, roomState?.roomId, inputRoomId, playerName, isAuthLoading]);
    ...
    return (
      ...
      <div style={flexibleContainerStyle}>
        ...
        >
          ...
          <div className="w-10 h-10 sm:w-14 sm:h-14 mx-auto mb-1 sm:mb-2 bg-slate-50 rounded-2xl flex items-center justify-center shadow-inner border border-slate-100 relative" onClick={handleLogoClick}>
            {isSpectator && (
            ...
            <div className="group">
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-100 px-3 py-1.5 sm:py-2.5 rounded-lg outline-none font-bold font-mono tracking-[0.2em] text-center transition-all focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white text-xs sm:text-sm disabled:bg-slate-100 disabled:opacity-70 disabled:cursor-not-allowed"
                />
                      ...
                      <div className="flex items-center gap-2">
                        {isHostInLobby && p.id !== socketService.playerId && (
                          <div className="flex items-center gap-1 hidden group-hover:flex">
                            <button
                              onClick={() => {
                                if (window.confirm(`确定要把 ${p.name} 降级为观众吗？`)) {
                                  socketService.demoteToSpectator(roomState.roomId, p.id);
                                }
                              }}
                            ...
                            <button
                              onClick={() => {
                                if (window.confirm(`确定要把 ${p.name} 踢出房间吗？`)) {
                                  socketService.kickPlayer(roomState.roomId, p.id);
                                }
                              }}
                      ...
                      <div className="flex items-center gap-1">
                        {isHostInLobby && (
                          <div className="flex items-center gap-1 hidden group-hover:flex">
                            <button
                              onClick={() => {
                                if (window.confirm(`确定要把 ${s.name} 升级为玩家吗？`)) {
                                  socketService.promoteToPlayer(roomState.roomId, s.id);
                                }
                              }}
                            ...
                            <button
                              onClick={() => {
                                if (window.confirm(`确定要把 ${s.name} 踢出房间吗？`)) {
                                  socketService.kickPlayer(roomState.roomId, s.id);
                                }
                              }}
      ...
      <div style={flexibleContainerStyle}>
        <div className="flex flex-col items-center justify-center h-full w-full bg-sky-100 text-[#0c4a6e]" onClick={() => document.documentElement.requestFullscreen().catch(() => {})}>
          <div className="w-16 h-16 bg-white/80 rounded-2xl shadow-xl shadow-sky-200 border border-white/50 flex items-center justify-center mb-6 animate-pulse">
            <img src={`/api/proxy-image?url=${encodeURIComponent('http://tdyuzzmmy.hn-bkt.clouddn.com/img/C3eURTs - Imgur.png')}`} alt="Catan" className="w-10 h-10 object-contain opacity-70" />
          </div>
      ...
      <header className="w-full flex items-center bg-white border-b border-black/5 px-2 z-50 overflow-hidden" style={{ height: headerHeight }}>
        ...
        <div className="flex items-center gap-2 lg:gap-3 shrink-0 cursor-default lg:absolute lg:left-2">
          <div className="flex items-center gap-2 lg:gap-3 relative" onClick={handleLogoClick}>
            ...
            <div className="flex flex-col items-start leading-none pr-4 border-r border-black/5 relative overflow-hidden">
              {/* Spectator logo mask */}
              {isSpectator && (
                <div className="absolute inset-0 bg-transparent pointer-events-none z-0" />
              )}
              <div className="relative z-10 flex flex-col items-start leading-none font-sans">
                ...
                <div className="flex items-center gap-1.5 grayscale-0">
                  ...
                  <div className="flex items-center gap-0.5 ml-1">
                    ...
                    <span className="text-[10px] font-mono font-black text-stone-500">{roomState?.spectators?.length || 0}</span>
                    {isSpectator && (
                      <button 
                        onClick={handleReturnToLobby}
                        className="ml-2 text-red-500 hover:text-red-600 transition-all transform active:scale-90 relative z-20"
          ...
          {gameStarted && (
            <div className={`absolute top-1 left-1 z-[50] flex flex-col gap-0.5 pointer-events-auto ${isMobile ? 'scale-90 origin-top-left' : ''}`}>
              {!isSpectator && (
                <button 
                  onClick={handleReturnToLobby}
                  className={`flex items-center gap-2 ${isMobile ? 'px-2 py-1 text-[9px] rounded-md' : 'px-5 py-2.5 text-[13px] rounded-lg'} bg-white/90 backdrop-blur-xl border border-black/5 text-stone-600 font-bold uppercase tracking-widest shadow-xl hover:bg-stone-100 transition-all transform active:scale-95 group pointer-events-auto`}
                  style={{ cursor: 'pointer' }}
                >
                  <LogOut size={isMobile ? 10 : 16} className="group-hover:-translate-x-0.5 transition-transform scale-x-[-1]" />
                  <span>中途离开</span>
                </button>
              )}
              ...
              {isHost && !isSpectator && (
                <button 
                  ...
                  className={`flex items-center gap-2 ${isMobile ? 'px-2 py-1 text-[9px] rounded-md' : 'px-5 py-2.5 text-[13px] rounded-lg'} bg-white/90 backdrop-blur-xl border border-black/5 text-red-500 font-bold uppercase tracking-widest shadow-xl hover:bg-red-500 hover:text-white transition-all transform active:scale-95 group`}
                >
                  <Trash2 size={isMobile ? 10 : 16} className="group-hover:rotate-12 transition-transform" />
                  <span>解散房间</span>
                </button>
              )}
            </div>
...
function HexCell({ hex, isSelected, isRobber, isPirate, onClick }: { hex: any, isSelected: boolean, isRobber: boolean, isPirate: boolean, onClick: () => void }) {
  ...
  useEffect(() => {
    ...
    switch (hex.type) {
      ...
      case HexType.Gold: src = '/api/proxy-image?url=' + encodeURIComponent('http://tdyuzzmmy.hn-bkt.clouddn.com/img/金矿4.jpg'); break;
      case HexType.Sea: src = '/api/proxy-image?url=' + encodeURIComponent('http://tdyuzzmmy.hn-bkt.clouddn.com/img/海洋7.png'); break;
    }
    if (src) {
      const img = new window.Image();
      img.src = src;
