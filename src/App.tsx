...
export default function App() {
  ...
  useEffect(() => {
    if (!roomState?.roomId || !gameStarted) return;
    const interval = setInterval(() => {
      socketService.requestSync(roomState.roomId);
    }, 15000); // 15 seconds
  ...
  }, [gameState?.tradeOffers, botProcessorId, reactToTrade]);
            ...
            <div className="flex flex-col gap-1.5 sm:gap-2 pt-1">
              {!roomState?.players.find(p => p.id === socketService.playerId)?.isReady ? (
                <button 
                  ...
                  className={`relative z-50 w-full bg-emerald-600 text-white py-2.5 sm:py-3.5 rounded-lg lg:rounded-xl font-black uppercase tracking-[0.2em] shadow-lg shadow-emerald-200 hover:bg-emerald-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-[10px] disabled:opacity-30 disabled:cursor-not-allowed disabled:grayscale ${isSpectator ? 'opacity-50 grayscale' : ''}`}
                >
