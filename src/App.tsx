...
function BuildItem({ id, icon, label, cost, onClick, active, disabled, compact, isDevCard }: { id?: string, icon: React.ReactNode, label: string, cost: Record<string, number>, onClick?: () => void, active?: boolean, disabled?: boolean, compact?: boolean, isDevCard?: boolean }) {
  return (
    <button 
      id={id}
      onClick={onClick}
      disabled={disabled}
      className={`w-full flex items-center justify-between ${compact ? 'py-1 px-2' : 'py-3 px-4'} rounded-xl border transition-all ${disabled ? '' : 'group'} ${active ? 'bg-emerald-50 border-emerald-400 scale-[1.01]' : `bg-white border-black/5 ${disabled ? '' : 'hover:border-black/20 hover:shadow-xl'}`} ${disabled ? 'cursor-not-allowed text-black' : ''} flex-1 min-h-0 min-w-0 lg:max-h-20`}
    >
      <div className={`flex items-center ${compact ? 'gap-1.5' : 'gap-3'} min-w-0`}>
        <div className={`${compact ? 'w-6 h-6 sm:w-7 sm:h-7' : 'w-10 h-10'} shrink-0 rounded-lg flex items-center justify-center transition-all duration-300 ${active ? 'bg-emerald-500 text-white' : (disabled ? 'bg-stone-100 text-stone-400' : 'bg-black text-white')}`}>
          {React.cloneElement(icon as React.ReactElement<any>, { size: compact ? 12 : 18 })}
        </div>
        <div className="flex flex-col items-start min-w-0">
          <span className={`${compact ? 'text-[8px] sm:text-[9px]' : 'text-sm'} font-black uppercase tracking-widest truncate w-full`}>{label}</span>
        </div>
      </div>
