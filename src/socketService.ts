...
class SocketService {
  ...
  connect() {
    ...
    this.socket.on('connect', () => {
      ...
      const playerName = localStorage.getItem('catan_player_name') || `Player_${this.playerId.substring(0, 4)}`;
      if (activeRoom) {
        console.log(`[Socket] Auto-rejoining room: ${activeRoom}`);
        this.emit('join_room', activeRoom, this.playerId, playerName);
      }
    });
