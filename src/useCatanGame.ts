...
export function useCatanGame() {
  ...
  const distributeResources = useCallback((hexes: Hex[], mapType: MapType, playerCount: number) => {
      ...
      let fullNumberPool: number[] = [];
      const baseNumbers = [2, 3, 3, 4, 4, 5, 5, 6, 6, 8, 8, 9, 9, 10, 10, 11, 11, 12];
      
      if (countTarget <= baseNumbers.length) {
        fullNumberPool = [...baseNumbers].sort(() => Math.random() - 0.5).slice(0, countTarget);
      } else {
        fullNumberPool = [...baseNumbers];
        const extraNeeded = countTarget - baseNumbers.length;
        const fillerPool = [2, 3, 4, 5, 6, 8, 9, 10, 11, 12, 3, 4, 5, 9, 10, 11]; 
        for (let i = 0; i < extraNeeded; i++) {
          fullNumberPool.push(fillerPool[i % fillerPool.length]);
        }
      }
      ...
      while (!success && attempts < 100) {
        attempts++;
        let possible = true;
        const currentHexes = allHexesNeedingNumbers.map(h => ({...h, id: h.id, q: h.q, r: h.r, isMainland: h.isMainland, isIsland: h.isIsland, number: null}));
        
        // In archipelago mode, we partition the pool to ensure balanced island spreads
        let pool = [...fullNumberPool].sort(() => Math.random() - 0.5);
        const mainlandDraft = currentHexes.filter(h => h.isMainland);
        const islandDraft = currentHexes.filter(h => h.isIsland);

        if (mapType === 'archipelago') {
          // 1. Calculate Mainland Red numbers (proportional, approx 22% of mainland)
          // For 3-4 players (18-19 hexes), this results in 4 red numbers (2x6, 2x8)
          const mainlandRedCount = Math.max(4, Math.floor(mainlandDraft.length * 0.22));
          const mainlandReds: number[] = [];
          for (let i = 0; i < mainlandRedCount; i++) mainlandReds.push(i % 2 === 0 ? 6 : 8);
          
          // 2. Prepare Island number pool (should be uniformly distributed 2-12, except 7)
          const islandTargetCount = islandDraft.length;
          const possibleIslandNumbers = [2, 3, 4, 5, 6, 8, 9, 10, 11, 12];
          const islandPool: number[] = [];
          for (let i = 0; i < islandTargetCount; i++) {
            islandPool.push(possibleIslandNumbers[i % possibleIslandNumbers.length]);
          }
          // Shuffle island pool to make individual islands varied
          islandPool.sort(() => Math.random() - 0.5);

          // 3. Prepare remaining pool for Mainland normals
          const mainlandNormalsCount = mainlandDraft.length - mainlandRedCount;
          // We need to take what's left of the intended total distribution after accounting for mainland reds and islands
          // Or just generate a balanced set for mainland normals
          const mainlandNormalsPool: number[] = [];
          const possibleNormals = [2, 3, 4, 5, 9, 10, 11, 12];
          for (let i = 0; i < mainlandNormalsCount; i++) {
            mainlandNormalsPool.push(possibleNormals[i % possibleNormals.length]);
          }
          mainlandNormalsPool.sort(() => Math.random() - 0.5);

          // 4. Place mainland red numbers with spacing
          let mainlandRedsPlaced = 0;
          const shuffledMainland = [...mainlandDraft].sort(() => Math.random() - 0.5);
          for (const h of shuffledMainland) {
            if (mainlandRedsPlaced >= mainlandRedCount) break;
            const hasRedNeighbor = currentHexes.some(other => 
              other.number !== null && isRed(other.number) && areAdjacent(h as any, other as any)
            );
            if (!hasRedNeighbor) {
              const num = mainlandReds[mainlandRedsPlaced++];
              h.number = num;
            }
          }
          if (mainlandRedsPlaced < mainlandRedCount) continue;

          // 5. Place island numbers (handling red adjacency purely on islands)
          const islandReds = islandPool.filter(isRed);
          const islandNormals = islandPool.filter(n => !isRed(n));
          
          let islandRedsPlaced = 0;
          const shuffledIslands = [...islandDraft].sort(() => Math.random() - 0.5);
          for (const h of shuffledIslands) {
            if (islandRedsPlaced >= islandReds.length) break;
            const hasRedNeighbor = currentHexes.some(other => 
              other.number !== null && isRed(other.number) && areAdjacent(h as any, other as any)
            );
            if (!hasRedNeighbor) {
              h.number = islandReds[islandRedsPlaced++];
            }
          }
          if (islandRedsPlaced < islandReds.length) continue;

          // Fill neutrals for islands
          for (const h of islandDraft) {
            if (h.number === null) {
              h.number = islandNormals.pop()!;
            }
          }

          // 6. Fill remaining Mainland hexes
          for (const h of mainlandDraft) {
            if (h.number === null) {
              h.number = mainlandNormalsPool.pop()!;
            }
          }
        } else {
          // Standard map placement (Original logic)
          const reds = pool.filter(isRed);
          const normals = pool.filter(n => !isRed(n));
          const shuffledHexes = [...currentHexes].sort(() => Math.random() - 0.5);
          let redsPlaced = 0;
          for (const h of shuffledHexes) {
            ...
            if (!hasRedNeighbor) h.number = reds[redsPlaced++];
          }
          if (redsPlaced < reds.length) continue;
          for (const h of currentHexes) {
            if (h.number === null) h.number = normals.pop()!;
          }
        }

        success = true;
        allHexesNeedingNumbers.forEach((h, i) => {
      ...
      if (!success) {
        // Fallback: simple greedy placement but still try to avoid red adjacency
        const shuffledHexes = [...allHexesNeedingNumbers].sort(() => Math.random() - 0.5);
        const fallbackPool = [...fullNumberPool];
        shuffledHexes.forEach(h => {
          const numIdx = fallbackPool.findIndex(n => {
            if (isRed(n)) {
              return !allHexesNeedingNumbers.some(other => other.number !== null && isRed(other.number) && areAdjacent(h as any, other as any));
            }
            return true;
          });
          h.number = numIdx !== -1 ? fallbackPool.splice(numIdx, 1)[0] : (fallbackPool.pop() || 2);
        });
      }
